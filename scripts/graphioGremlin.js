...
var graphioGremlin = (function () {
    ...
    function advanced_search_query(gremlin_query) {

        if ($.trim(gremlin_query) != "") {
            //TODO: support for SINGLE_COMMANDS_AND_NO_VARS = true
            $('#messageArea').html('<h3>(loading)</h3>');
            var message = "Query: '" + gremlin_query + "'";
    ...
    function search_query() {
        ...
        // while busy, show we're doing something in the messageArea.
        $('#messageArea').html('<h3>(loading)</h3>');
        var message = "Query: '" + filtered_string + "'"
        if (SINGLE_COMMANDS_AND_NO_VARS) {
    ...
    function run_ajax_request(gremlin_query, query_type, active_node, message, callback) {
        ...
        $.ajax({
            ...
            failure: function (msg) {
                console.log("failed");
                $('#outputArea').html("Can't access database");
                $('#messageArea').html('');
            },
            error: function (data) {
                console.log("error");
                $('#outputArea').html("Invalid search");
                $('#messageArea').html('');
            }
        });
    }
    ...
    function run_websocket_request(gremlin_query, query_type, active_node, message, callback) {
        ...
        ws.onerror = function (err) {
            ...
            console.log(err);
            $('#outputArea').html("Connection error");
            $('#messageArea').html('');
